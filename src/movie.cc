...
int Movie::MovieFileInit(int* fd, char* MovieFileName, int mode)
  {
	...
	case O_RDWR:
	  ...
	  if (! (creator = new char[creator_len])) {
            cerr<<"Out of memory"<<endl;
          }
	...
	case O_WRONLY|O_CREAT:
  	  *fd = open(MovieFileName, mode, 0640);
	  if (*fd==-1) {perror("movie.cc 508: Movie::MovieFileInit");return -1;}
...
#define PATCHLEVEL 0
	  ...
	  char tmpch;
	  tmpch=(char)0;
	  i = ::write(*fd,&tmpch,1);	// length=0, no creator data
	  if (i == -1)
	  {
